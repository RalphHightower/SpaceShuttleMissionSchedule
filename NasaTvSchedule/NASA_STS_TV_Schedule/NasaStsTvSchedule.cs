...
{
	/// <summary>
	/// Enum to interpret the different types of rows in the spreadsheet
	/// empty: blank
	...
	/// </summary>
	public enum ScheduleType { empty, columnHeading, dateHeading, flightDayHeading, scheduleEntry, definitionOfTerms, error };
	...
	public class NasaStsTVSchedule : Object
    {
        ...
        private Regex rgDateHeader
        {
            get
            {
                ...
                {
                    crgDateHeader = new Regex(Properties.Resources.RG_DATE_HEADER, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
                return (crgDateHeader);
            }
            set { crgDateHeader = value; }
        }
        /// <summary>
        ...
        private Regex rgEvaActivity
        {
            get
            {
                ...
                {
                    crgEvaActivity = new Regex(Properties.Resources.RG_EVA_ACTIVITY, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
                return (crgEvaActivity);
            }
            set { crgEvaActivity = null; }
        }
        /// <summary>
        ...
        private Regex rgFlightDayHeader
        {
            get
            {
                ...
                {
                    crgFlightDayHeader = new Regex(Properties.Resources.RG_FLIGHDAY_HEADER, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
                return (crgFlightDayHeader);
            }
            set { crgFlightDayHeader = value; }
        }
        /// <summary>
        ...
        private Regex rgFlightDayHighlights
        {
            get
            {
                ...
                {
                    crgFlightDayHighlights = new Regex(Properties.Resources.TM_RG_FLIGHT_DAY_HIGHLIGHTS, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
                return (crgFlightDayHighlights);
            }
            set { crgFlightDayHighlights = value; }
        }
        /// <summary>
        ...
        private Regex rgIssCrewSleepActivity
        {
            get
            {
                ...
                {
                    crgIssCrewSleepActivity = new Regex(Properties.Resources.RG_ISS_CREW_SLEEP_ACTIVITY, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
        ...
        private Regex rgShuttleCrewSleepActivity
        {
            get
            {
                ...
                {
                    crgShuttleCrewSleepActivity = new Regex(Properties.Resources.RG_SHUTTLE_CREW_SLEEP_ACTIVITY, RegexOptions.Compiled | RegexOptions.CultureInvariant);
                }
                return (crgShuttleCrewSleepActivity);
            }
            set { crgShuttleCrewSleepActivity = value; }
        }
        #endregion
		...
		private string[,] EventTimes = {
            ...
            //{ Properties.Resources.TM_CREW_SLEEP_BEGINS, "08:30:00" },    // ReadAhead will detect new rgSubjectVerbPattern wake period
        ...
        /// I haven't figured out how to open a file using Microsoft.Office.Tools.Excel
        /// </summary>
		...
		public NasaStsTVSchedule(string excelFile, string viewingTimeZone, ExcelInterface typeInterface)
		{
			...
			switch (typeInterface)
			{
				...
				case ExcelInterface.ToolsExcel:
					...
					break;
				default:
					throw new ArgumentException(Properties.Resources.ERR_INVALID_ARGUMENT, Properties.Resources.ERR_ARGUMENT_TYPE_EXCEL);
			}
			PostInitializeExcel();
		}
        ...
        public NasaStsTVScheduleEntry ReadScheduleRow()
        {
            ...
            if (SuccessfullyOpened)
            {
                for (; !EOF() && entryType != ScheduleType.scheduleEntry; CurrentRow++)
                {
			...
			{
				dataRow = new NasaStsTVScheduleEntry(DateTime.MinValue, DateTime.MinValue, false, ProcessingError.Message, 0, "", "", ScheduleType.error);
			}
        ...
        private void GetCreationRevisionDate()
        {
            ...
            if ((cellOne != null) && (cellOne.GetType().ToString() == Properties.Resources.SYSTEM_STRING))
            {
                ...
                Regex rgDate = new Regex(Properties.Resources.RG_MM_DD_YY);
                    ...
                    grpcolDate[Properties.Resources.IX_YEAR].Success)
                {
                    ...
                    DateTime dtRevisionDate = DateTime.ParseExact(revisionDate, Properties.Resources.NASA_MM_DD_YY, CultureInfo.InvariantCulture);
        ...
        private NasaStsTVScheduleEntry ProcessEntry(int row)
        {
                ...
                && (HeadingDate.Hour == 0) && (HeadingDate.Minute == 0)))
            {
                ...
                //  State variables for docking and in space are not reliable for schedule revisions published after launch or docking
                ...
                if (TvScheduleCells.GetValue(row, CentralTimeColumnHeader) != null)
                {
                    ...
                    validEntry = true;
                    //  This situation may not occur (except for STS Landing since though there are next events, the events are Net Landing + a time span
                    //  This was originally a kludge for the hour transition period for falling back to Standard Time
                    if (dtBeginViewingTime > dtEndViewingTime)
                        dtEndViewingTime = dtBeginViewingTime.AddHours(1);   //  If the end time occurs before the beginning time, assume 30 minutes
                    entryRow = new NasaStsTVScheduleEntry(dtBeginViewingTime, dtEndViewingTime, Changed, Subject, Orbit, Site, FlightDay, ScheduleType.scheduleEntry);
                }
            }
		...
		/// <summary>
		/// Some NASA STS TV Schedules have the event on multiple lines instead in a single cell
		/// This routine reads ahead concatenating the lines until an empty cell is found or until the end of the spreadsheet is found
		/// </summary>
		...
		private DateTime GuesstimateFixedEvents(string subject, DateTime beginTime)
		{
			...
			//  Some events do not last until the next event, such as flight missionDay highlights, press conferences, interviews, etc.
			//  This uses some rough estimates for those events.
			//  The guesstimate for the event times are recorded in the resources file as comments
			//  A TO DO for me (Ralph Hightower) is to create a method to read from the resource file and get the times from the comments
			//  Expediency ruled in favor of duplicating the information in a two dimensional string array
        ...
        private DateTime ReadAhead()
        {
            ...
            for (indexRowAhead = CurrentRow + 1; (indexRowAhead <= RowCount) && (scheduleRow != ScheduleType.scheduleEntry) && 
				! tempEOF && ! ErrorProcessed; indexRowAhead++)
            {
                ...
                else
                {
                    ...
                    if (cellOrbit != null)
                    {
                        ...
                        if (cellOrbitType.FullName == Properties.Resources.SYSTEM_STRING)
                        {
                            ...
                            //  Don't loop through days for end of file record
                            if (endDateHeader.Contains(Properties.Resources.NASA_DEFINITION_OF_TERMS))
                            {
                                tempEOF = true;
                                break;
                            }
                            //  Don't loop through days if a Header Record
                            if (endDateHeader == Properties.Resources.NASA_ORBIT)
                                continue;
                            //  Don't loop through days if a Flight Day Record
                ...
                {
					if (!TvScheduleCells.GetValue(indexRowAhead, SubjectColumnHeader).ToString().Contains(Properties.Resources.NASA_CREW_WAKE_UP))
					{
                ...
                case "System.String":
                    {
                        ...
                        //  Row contains "DEFINITION OF TERMS" which is the end of file; no schedule entries exist after this value
                        //  what remains are the definitions of the acronyms used in the schedule
                        ...
                        else
                        {
							if (MatchDateHeader(cellOrbitValue))
							{
								...
								break;  //  Have the Date, no need to check any other days
							}
        ...
        private bool ProcessOrbitHeader(int row)
        {
            ...
            if (headingCount < 7)
            {
                string InvalidOrbitHeader = Properties.Resources.ERR_INVALID_COLUMN_HEADER + row.ToString(CultureInfo.CurrentCulture);
		...
		private string ExcelFormatTime(int row, int cell)
		{
			...
			switch (ExcelTypeInterface)
			{
				case ExcelInterface.InteropExcel:
					...
					formattedTime = InteropExcelIF.FormatTime(TvScheduleCells, row, cell);
					break;
				case ExcelInterface.ToolsExcel:
					formattedTime = ToolsExcelIF.FormatTime(TvScheduleCells, row, cell);
					break;
				default:
					throw new ArgumentException(Properties.Resources.ERR_EXCEL_FORMAT_TIME, Properties.Resources.ERR_ARGUMENT_TYPE_EXCEL);
			}
        ...
        /// There is a kludge for that 2 AM hour that does not occurr when Daylight Savings Time ends and Standard Time begins for Eastern Time
        /// 
        ...
        private DateTime ConvertFromCentralTzToViewingTz(DateTime dtConvert, string timeOfday)
        {
			...
			DateTime dtEventPart = DateTime.Parse(convertTime, CultureInfo.CurrentCulture);
            DateTime dtCentralKludge = new  DateTime(dtConvert.Year, dtConvert.Month, dtConvert.Day, dtEventPart.Hour, dtEventPart.Minute, dtEventPart.Second, dtEventPart.Millisecond);
            DateTime dtCentralUtc = TimeZoneInfo.ConvertTimeZoneToUtc(dtCentralKludge, JohnsonSpaceCenterTZ);
            DateTime dtCentral = TimeZoneInfo.ConvertUtcToTimeZone(dtCentralUtc, JohnsonSpaceCenterTZ);
            DateTime dtViewingTZKludge = TimeZoneInfo.ConvertTimeZoneToTimeZone(dtCentral, JohnsonSpaceCenterTZ, ViewingTimeZoneTZ);
            //  Need a kludge here for when CentralTime == Eastern Time
            DateTime dtViewingTZ = new DateTime(dtViewingTZKludge.Year, dtViewingTZKludge.Month, dtViewingTZKludge.Day, dtViewingTZKludge.Hour, dtViewingTZKludge.Minute, dtViewingTZKludge.Second, DateTimeKind.Local);
            //  The code below fixes the transition time when Central Time hasn't encountered the time change and Eastern Time has
            if ((dtCentral.Hour == dtViewingTZKludge.Hour) && (ViewingTimeZoneTZ.DisplayName == Properties.Resources.TZ_US_EASTERN))
        ...
        {
            bool shuttleCrewSleep = SubjectVerbPatternMatch(rgShuttleCrewSleepActivity, Properties.Resources.IX_SHUTTLE, Properties.Resources.NASA_CREW_SLEEP_BEGINS, row);
        ...
        {
            bool shuttleCrewWakeUp = SubjectVerbPatternMatch(rgShuttleCrewSleepActivity, Properties.Resources.IX_SHUTTLE, Properties.Resources.NASA_CREW_WAKE_UP, row);
        ...
        {
            bool issCrewSleep = SubjectVerbPatternMatch(rgIssCrewSleepActivity, Properties.Resources.IX_ISS, Properties.Resources.NASA_CREW_SLEEP_BEGINS, row);
        ...
        {
            bool issCrewWakeUp = SubjectVerbPatternMatch(rgIssCrewSleepActivity, Properties.Resources.IX_ISS, Properties.Resources.NASA_CREW_WAKE_UP, row);
		...
		private bool EVABegins(int row)
		{
			...
			{
				evaBegins = SubjectVerbPatternMatch(rgEvaActivity, Properties.Resources.IX_EVA, Properties.Resources.NASA_BEGINS, row);
			}
		...
		{
            bool evaEnds = SubjectVerbPatternMatch(rgEvaActivity, Properties.Resources.IX_EVA, Properties.Resources.NASA_ENDS, row);
		...
		/// 
		/// </summary>
		private class InteropExcelInterface : Object
		{
			/// <summary>
			/// Carriage Return, Line Feed
			/// </summary>
			...
			private InteropExcel.ApplicationClass InteropExcelApplication
			{
				...
				set { m_InteropExcelApplication = value; }
			}
			/// <summary>
			...
			private InteropExcel.WorkbookClass InteropExcelWorkbook
			{
				...
				set { m_InteropExcelWorkbook = value; }
			}
			/// <summary>
			...
			private InteropExcel.Sheets InteropExcelSheets
			{
				...
				set { m_InteropExcelSheets = value; }
			}
			/// <summary>
			...
			private InteropExcel.Worksheet InteropExcelWorksheet
			{
				...
				set { m_InteropExcelWorksheet = value; }
			}
			/// <summary>
			/// Interop Excel Range
			/// </summary>
			...
			public System.Array OpenExcelFile(string NasaTVScheduleFile)
			{
				...
				{
					InteropExcelApplication = new Microsoft.Office.Interop.Excel.ApplicationClass();
					InteropExcelWorkbook = (InteropExcel.WorkbookClass)InteropExcelApplication.Workbooks.Open(NasaTVScheduleFile, false, true, Type.Missing, Type.Missing, Type.Missing,
				...
				{
					if (comException.TargetSite.Name == Properties.Resources.COM_EXCEPTION_INTEROPEXCEL_OPENEXCELFILE_GETRANGE)
					{
		...
		private class ToolsExcelInterface : Object
		{
			/// <summary>
			/// Carriage Return, Line Feed
			/// </summary>
			...
			private ToolsExcel.Workbook ToolsExcelWorkbook
			{
				...
				set { m_ToolsExcelWorkbook = value; }
			}
			/// <summary>
			...
			private ToolsExcel.Worksheet ToolsExcelWorksheet
			{
				...
				set { m_ToolsExcelWorksheet = value; }
			}
			/// <summary>
			/// Tools Excel Range
			/// </summary>
			...
			private ToolsExcel.NamedRange ToolsExcelNamedRange
			{
				...
				set { m_ToolsExceNamedlRange = value; }
			}
			...
			{
				string formattedTime = "";
				Microsoft.Office.Interop.Excel.ApplicationClass excelApplication = (Microsoft.Office.Interop.Excel.ApplicationClass)ToolsExcelWorkbook.Application;
	...
	public class NasaStsTVScheduleEntry : Object
	{
		...
		public NasaStsTVScheduleEntry(DateTime entryBeginDateTime, DateTime entryEndDateTime, bool entryRevised, string entrySubject, double entryOrbit, string entrySite, string entryFlightDay, ScheduleType type)
		{
